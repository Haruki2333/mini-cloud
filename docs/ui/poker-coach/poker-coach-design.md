...
## 10. 确认页（`confirm.html`）行内编辑约定

确认页是快速录入通道的"扫一眼校对"页：单屏滚动、不分步、所有字段都行内可编辑。

- 卡片之间使用虚线分隔（不要 box-shadow 模糊）；每张卡 ±0.3deg 微旋转
- chip / segmented / card-slot 直接行内可点改，复用 [`widgets.js`](../../../backend/demo/poker-coach/js/widgets.js) 提供的选牌器
- 每条街展示为一行文本概述（如 "UTG raise 3bb · BTN call"），右下"✎ 编辑"按钮就地展开为完整 action-builder
- **缺字段红笔波浪线**：后端 `/parse` 返回的 `missing[]` 中每个字段路径对应 DOM 上 `data-field="<path>"` 的元素，加 `class="missing"`；CSS 用红笔波浪线（容器型元素使用底部红色点划线伪元素，避免对内部按钮造成视觉污染）
- **warnings 顶部带**：荧光笔黄色背景 + 斜体衬线小字
- **等待解析期**：不要骨架屏/旋转器；首页提交后的过渡用一句手写字 "coach is reading…"
- 主按钮"保存并复盘"在底栏；缺关键字段（盲注 / Hero 位置 / 起手牌 / 翻前 actions）时按钮禁用