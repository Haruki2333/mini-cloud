...
│   │   │   ├── llm.js              # LLM 调用封装（chat 非流式 + chatStream 流式，MODEL_REGISTRY 注册 6 款模型，均走 lingyaai 代理）
...
- 编写后端代码时，在请求入口、业务分支、外部调用、异常捕获等关键位置输出带 `[模块名]` 前缀的日志，便于线上定位问题
